...
function wrapReturnStatements(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!inReturnBlock) {
            ...
            // 检查是否是 return result; 或 return decode; 等单行 return
            else if (line.match(/^\s*return\s+\w+\s*;/)) {
                // 将 return xxx; 改为 return { data: xxx };
                result.push(line.replace(/return\s+(\w+)\s*;/, 'return { data: $1 };'));
            }
            // 检查是否是单行 return { ... };
            else if (line.match(/^\s*return\s*\{.*\}\s*;/)) {
                // 将 return { ... }; 改为 return { data: { ... } };
                result.push(line.replace(/return\s*\{/, 'return { data: {').replace(/\}\s*;/, ' };'));
            }
            else {
                result.push(line);
            }
